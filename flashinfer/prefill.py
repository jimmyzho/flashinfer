...
def trtllm_fmha_v2_prefill(
    ...
    if mask_mode is not None and mask_mode.lower() == "custom":
        raise NotImplementedError(
            "custom-mask FMHAv2 kernels are not generated by the JIT path "
            "(no API plumbs the packed custom-mask tensor); use padding, "
            "causal, sliding_window, or chunked."
        )
    ...
    # Narrow JIT codegen to this call's configuration: query's last dim is
    # head_dim_qk for every layout (packed [T,3,H,D], others [T,H,D]), and
    # the remaining flags are known here, so the free function always uses
    # the small per-config module (same URI as a wrapper plan() with
    # head_dim passed).
    module = get_trtllm_fmha_v2_module(
        ...
        query.shape[-1],
        use_alibi=(
            pos_encoding_mode is not None and pos_encoding_mode.lower() == "alibi"
        ),
        use_logits_soft_cap=bool(softcapping_scale),
        enable_skip_softmax=bool(skip_softmax_threshold_scale_factor),
    )
